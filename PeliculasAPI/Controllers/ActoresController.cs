...
namespace PeliculasAPI.Controllers
{
    ...
    public class ActoresController:ControllerBase
    {
        ...
        public async Task<ActionResult<List<PeliculasActorDTO>>> BuscarPorNombre(string nombre = "")
        {
            if (string.IsNullOrWhiteSpace(nombre))
            {
                return new List<PeliculasActorDTO>();
            }

            return await context.Actores
                .Where(x => x.Nombre.Contains(nombre))
                .OrderBy(x => x.Nombre)
                .Select(x => new PeliculasActorDTO { Id = x.Id, Nombre = x.Nombre, Foto = x.Foto })
                .Take(5)
                .ToListAsync();
        }
